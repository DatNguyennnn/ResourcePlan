...
export default function ProjectsPage() {
  ...
  const getStatusColor = (status: string) => {
    if (status.includes('Đã có hợp đồng')) return 'bg-green-100 text-green-700 dark:bg-blue-900/40 dark:text-blue-400';
      ...
      <main className="flex-1 p-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-700">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300">Mã DA</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300">Tên dự án</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300">Mô tả</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300">PM</th>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center animate-backdropIn">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-lg border border-slate-200 dark:border-slate-700 animate-modalIn">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                    className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-700 dark:text-slate-100" rows={3} />
                </div>
                <div>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 border border-slate-300 dark:border-slate-600 py-2 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 transition-colors cursor-pointer">Hủy</button>
              </div>
            </form>
          </div>
        )}
